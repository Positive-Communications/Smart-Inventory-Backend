...
const proccessScan = async (tag: Tags, gate: Gate, carrier: Carrier) => {

    const productManager = new ProductManager();

    let prev = tag.previousScan || null;

    const moves = await getConnection().manager.find(Move);

    if (prev === null || prev === undefined) {

        let updated = await getConnection().createQueryBuilder()
            .update(Tags)
            .set({ scan: gate, })
    ...
    let newTag = await readTagByEPC(tag.epc);
